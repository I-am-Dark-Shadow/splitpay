...
export default function GroupDetails() {
  ...
  const [isGlobalSettled, setIsGlobalSettled] = useState(false); // State kept for other UI logic if needed
  ...
  const calculateStats = (groupData) => {
    ...
    // Logic for "Settled" badge can stay, but won't disable buttons
    setIsGlobalSettled(groupData.expenses.length > 0 && settlements.length === 0);
  };
    ...
    <div className="min-h-screen bg-slate-50 pb-24 pt-8 relative">
      ...
      <div className="sticky top-0 z-40 bg-slate-50/90 backdrop-blur border-b border-slate-200 px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Add Member Button (Always Enabled) */}
          <button 
          ...
          {/* ✅ Add Expense Button (FIXED: Always Enabled) */}
          <Link 
            to={`/groups/${id}/add-expense`}
            className="h-9 px-3 rounded-xl bg-slate-900 text-white text-xs font-semibold flex items-center gap-1 hover:bg-slate-700 active:scale-95 transition-all"
          >
          ...
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-sm p-6 animate-in fade-in zoom-in duration-200 border-2 border-rose-100">
            <div className="flex flex-col items-center text-center">
              ...
              <div className="flex gap-3 w-full">
                <button onClick={() => setShowDeleteModal(false)} className="flex-1 py-3 rounded-xl bg-slate-100 text-slate-700 font-bold text-sm hover:bg-slate-200 transition-colors">Cancel</button>
                <button onClick={confirmDeleteGroup} disabled={deleting} className="flex-1 py-3 rounded-xl bg-rose-600 text-white font-bold text-sm hover:bg-rose-700 shadow-lg shadow-rose-200 transition-all active:scale-95 disabled:opacity-70">
